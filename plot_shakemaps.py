...
parser.add_argument("filename", help="output from ComputeGroundMotionParametersFromSurfaceOutput_Hybrid.py")
...
def custom_terrain_cmap():    
    ...
    cmap_new = mpl.colors.ListedColormap(cmap_new, name='custom_terrain1', N=cmap_new.shape[0])
    return cmap_new
...
def plot_shakemaps():
    ...
    minor_ticks = np.unique(np.concatenate([np.linspace(1,10,10)*i for i in [0.001, 0.01, 0.1,1,10]]))
    major_ticks = np.unique(np.concatenate([np.array((1,2,5,10))*i for i in [0.001, 0.01, 0.1,1,10]]))

    cbar_levels = [get_cbar_level(data[i], major_ticks) for i in range(3)]


    # remove edges that may be affected from the projection
    xlim=[np.min(x)+(np.max(x)-np.min(x))*0.03, np.max(x)-(np.max(x)-np.min(x))*0.03]
    ylim=[np.min(y)+(np.max(y)-np.min(y))*0.03, np.max(y)-(np.max(y)-np.min(y))*0.03]
    ...
    for i in range(len(data)):
        tri = ax[int(i/2), i%2].tricontourf(x[::step], y[::step], data[i][::step], cmap=cmaps[1], extend="both",
        ...
        major_ticks_tmp = major_ticks[(major_ticks <= cbar_levels[i][-1]) & (major_ticks >= cbar_levels[i][0])]
        minor_ticks_tmp = minor_ticks[(minor_ticks <= cbar_levels[i][-1]) & (minor_ticks >= cbar_levels[i][0])]
        cbar.set_ticks(major_ticks_tmp)
        cbar.set_ticks(minor_ticks_tmp, minor=True)


    # plot topography
    tri = ax[1, 1].tricontourf(x[::step], y[::step], z[::step], cmap=custom_terrain_cmap(), extend="both")
    ...
    plt.savefig("shakemap.jpg", dpi=300, facecolor='white', transparent=False, bbox_inches="tight")
    plt.close()   
    print("Plotting done.")
